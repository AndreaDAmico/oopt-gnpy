...
from numpy import interp, pi, zeros, shape, where, cos, array, append, ones, argsort, exp, arange, sqrt, empty, trapz, \
    arcsinh, clip, abs, sum, concatenate, unique, flip, outer, inner, transpose, max, format_float_scientific, \
    argwhere, prod, diag, sin, log, reshape
...
def raised_cosine(abscissa, centrum, support, roll_off):
    """Returns a raised cosine shaped normalized PSD (values in [0,1])

    @:param abscissa: evaluation variable values
    @:param centrum: raised cosine centrum
    @:param support: equivalent zero roll off raised cosine support
    @:param roll_off: raised cosine beta coefficient
    @:return: normalized psd
    """
    abscissa = abs(abscissa - centrum)  # base band module
    pass_band = support/2 * (1 - roll_off)
    stop_band = support/2 * (1 + roll_off)
    psd = zeros(abscissa.shape)
    psd += 1 * (abscissa <= pass_band)
    if roll_off != 0:
        transient = (pass_band < abscissa) * (abscissa < stop_band)
        psd[transient] = 0.5 * (1 + cos(pi / (support * roll_off) * (abscissa[transient] - support/2 * (1 - roll_off))))
    return psd
...
class RamanSolver:
    ...
    @staticmethod
    def calculate_attenuation_profile(spectral_info, fiber, sim_params):
        """This function evaluates the attenuation profile along the z axis for all the frequency propagating in the
        ...
        z = append(arange(0, fiber.params.length, z_resolution), fiber.params.length)
...
class NliSolver:
    """ This class implements the NLI models.
        ...
        'ggn_spectrally_separated': XPM plus SPM
    """
    ...
    @staticmethod
    def compute_nli(spectral_info, srs, fiber, sim_params):
        """ Compute the nonlinear interference noise generated by the spectrum propagation through the fiber

        @:param spectral_info: instance of gnpy.core.info.SpectralInformation
        @:param srs: stimulated raman scattering instance of gnpy.core.science_utils.StimulatedRamanScattering
        @:param fiber: instance of gnpy.core.elements.Fiber or gnpy.core.elements.RamanFiber
        @:param sim_params: instance of gnpy.core.parameters.SimParams
        @:return: nli evaluated at the fiber input
        """
        ...
        if 'gn_model_analytic' == sim_params.nli_params.method.lower():
            nli = NliSolver._gn_model_analytic(spectral_info, alpha, beta2, gamma, length)
        elif 'ggn_model_numeric' in sim_params.nli_params.method.lower():
            nli = NliSolver._ggn_model_numeric(spectral_info, srs, beta2, gamma, length)
    ...
    # Analytic method computing the NLI with the GN-model
    @staticmethod
    def _gn_model_analytic(spectral_info, alpha, beta2, gamma, length):
        """Evaluates the nonlinear interference noise using the GN model defined in
        `arXiv:1209.0394 <https://arxiv.org/abs/1209.0394>`__

        @:param spectral_info: instance of gnpy.core.info.SpectralInformation
        @:param alpha: power attenuation coefficient for f in frequency [Neper/m]
        @:param beta2: second order dispersion coefficient [1/(m * Hz^2)]
        @:param gamma: nonlinear interference coefficient [1/W/m]
        @:param length: fiber length [m]
        @:return: nonlinear interference noise evaluated at the fiber input
        """
        baud_rate = spectral_info.baud_rate
        psd = spectral_info.psd
        df = spectral_info.df
        ggg = outer(psd, psd ** 2)
        left_bound = df - 0.5 * baud_rate
        right_bound = df + 0.5 * baud_rate
        central = outer(baud_rate, ones(spectral_info.number_of_channels))
        effective_length = (1 - exp(- alpha * length)) / alpha

        # Frequency integral
        psi = 0.5 * effective_length**2 / (2 * pi * abs(beta2) / alpha) * \
            (arcsinh(pi**2 * abs(beta2) * central * right_bound / alpha) -
             arcsinh(pi**2 * abs(beta2) * central * left_bound / alpha))

        # Statistical weight
        spm_matrix = diag(ones(spectral_info.number_of_channels))
        xpm_matrix = ones(df.shape) - spm_matrix
        spm_weight = (16.0/27.0) * spm_matrix
        xpm_weight = 2 * (16.0/27.0) * xpm_matrix
        weight = spm_weight + xpm_weight

        # nli power spectral density evaluated on central frequencies
        psd_nli = gamma**2 * sum(weight * ggg * psi, 1)

        # Local white noise approximation
        nli = spectral_info.baud_rate * psd_nli
        return nli

    # Numericc methods computing the NLI with the GGN-model
    @staticmethod
    def _ggn_model_numeric(spectral_info, srs, beta2, gamma, length):
        """ Computes the nonlinear interference power on a single carrier.
        The method uses eq. 120 from arXiv:1209.0394.
        """
        g_cut = spectral_info.signal / spectral_info.baud_rate
        spm_weight = 1 * (64 / 81) * (5 / 6) * (6 / 8) * gamma ** 2
        xpm_weight = 2 * (64 / 81) * (5 / 6) * (4 / 8) * gamma ** 2
        ...
        for i, cut_carrier in enumerate(spectral_info.carriers):
            for pump_carrier in spectral_info.carriers:
                ...
                if dn == 0:  # SPM
                    ggg = g_cut[i] ** 3
                    g_nli[i] += spm_weight * ggg * NliSolver._ggn_numeric_psi(cut_carrier, cut_carrier, srs, beta2, length)
                else:  # XPM
                    g_pump = pump_carrier.power.signal / pump_carrier.baud_rate
                    ggg = g_cut[i] * g_pump ** 2
                    g_nli[i] += xpm_weight * ggg * NliSolver._ggn_numeric_psi(cut_carrier, cut_carrier, srs, beta2, length)

        nli = spectral_info.baud_rate * g_nli  # Local white noise
        return nli

    @staticmethod
    def _ggn_numeric_psi(cut_carrier, pump_carrier, srs, beta2, length):
        ...
        dz = z[1:] - z[:-1]
        alpha_z = - 2 * log(rho_pump[1:] / rho_pump[:-1]) / dz
        alpha = - 2 * log(rho_pump[-1] / rho_pump[0]) / z[-1]
        ...
        if cut_carrier.channel_number == pump_carrier.channel_number:  # SCI, SPM
            ...
            psi = 0.5 * arcsinh(pi ** 2 * asymptotic_length * abs(beta2) * cut_carrier.baud_rate *
                                (delta_f + 0.5 * pump_carrier.baud_rate))
            psi -= 0.5 * arcsinh(pi ** 2 * asymptotic_length * abs(beta2) * cut_carrier.baud_rate *
                                 (delta_f - 0.5 * pump_carrier.baud_rate))
    ...
    @staticmethod
    def _ggn_spectrally_separated(spectral_info, srs, alpha, beta2, beta3, f_ref_beta, gamma, sim_params):
        ...
        for cut_carrier in cuts:
            i = cut_carrier.channel_number - 1
                    ...
                    if delta_f == 0:
                        ...
                        g_nli_computed += \
                            xpm_weight * ggg * NliSolver._fast_generalized_psi(f_eval, cut_carrier, pump_carrier,
                                                                               f_cut_resolution, srs, alpha[i], beta2,
                                                                               beta3, f_ref_beta)
    ...
    def _fast_generalized_psi(f_eval, cut_carrier, pump_carrier, f_cut_resolution, srs, alpha, beta2, beta3,
        ...
        for f1_index, f1 in enumerate(f1_array):
            ...
            integrand_f2 = generalized_rho_nli
