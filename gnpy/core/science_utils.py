...
class NliSolver:
    ...
    def compute_nli(spectral_info, srs, fiber, sim_params):
        ...
        elif 'ggn_fast_approx' in sim_params.nli_params.method.lower():
            nli = NliSolver._ggn_fast_approx(spectral_info, srs, alpha, beta2, gamma, length)
        else:
    ...
    @staticmethod
    def _ggn_fast_approx(spectral_info, srs, alpha, beta2, gamma, length):
        g_cut = spectral_info.signal / spectral_info.baud_rate
        spm_weight = (16.0 / 27.0) * gamma ** 2
        xpm_weight = 2 * (16.0 / 27.0) * gamma ** 2
        g_nli = zeros(spectral_info.frequency.shape)
        for i, cut_carrier in enumerate(spectral_info.carriers):
            for j, pump_carrier in enumerate(spectral_info.carriers):
                ...
                if dn == 0:  # SPM
                    ggg = g_cut[i] ** 3
                    g_nli[i] += spm_weight * ggg * NliSolver._ggn_fast_psi(cut_carrier, pump_carrier,
                                                                           alpha[j], beta2, length)
                else:  # XPM
                    g_pump = pump_carrier.power.signal / pump_carrier.baud_rate
                    ggg = g_cut[i] * g_pump ** 2
                    g_nli[i] += xpm_weight * ggg * NliSolver._ggn_fast_psi(cut_carrier, pump_carrier,
                                                                           alpha[j], beta2, length)
        nli = spectral_info.baud_rate * g_nli  # Local white noise
        return nli

    @staticmethod
    def _ggn_fast_psi(cut_carrier, pump_carrier, alpha, beta2, length):
        effective_length = NliSolver.effective_length(alpha, length)
        ...
        # XPM
            ...
            generalized_psi = 2 * effective_length / delta_beta * arctan((delta_beta * Rs_cut / 2) / alpha) * Rs_pump
