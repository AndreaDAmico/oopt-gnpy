...
def test_create_arbitrary_spectral_information():
    ...
    si = create_arbitrary_spectral_information(frequency=[193.35e12, 193.3e12, 193.25e12],
                                               baud_rate=32e9, signal=[1, 2, 3])
    assert_array_equal(si.signal, array([3, 2, 1]))

    with pytest.raises(SpectrumError, match='Spectrum baud rate larger than the slot width.'):
        create_arbitrary_spectral_information(frequency=[193.25e12, 193.3e12, 193.35e12], signal=1,
                                              baud_rate=64e9, slot_width=50e9)

    with pytest.raises(SpectrumError,
                       match='Spectrum required slot widths larger than the frequency spectral distances.'):
        create_arbitrary_spectral_information(frequency=[193.26e12, 193.3e12, 193.34e12], signal=1, baud_rate=40e9)
